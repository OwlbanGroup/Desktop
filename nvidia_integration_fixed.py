...
class NvidiaIntegration:
    ...
    def get_driver_updates(self) -> Dict[str, Any]:
        """
        Fetch driver updates and information from NVIDIA

        Returns:
            Dict containing driver versions, download links, system requirements, and release information
        """
        url = "https://www.nvidia.com/en-us/drivers/"
        try:
            logger.info("Fetching driver updates from NVIDIA drivers page")
            response = requests.get(url, timeout=10)
            response.raise_for_status()
            soup = BeautifulSoup(response.text, "html.parser")
            ...
            # Look for version numbers in various formats
            ...
            req_sections = soup.find_all(["div", "section"], class_=re.compile(r"(req|system|spec)", re.I))
            ...
            logger.error(f"Error fetching driver updates: {e}")
            return {
...
# Helper function to get NVIDIA Control Panel instance
def get_nvidia_control_panel():
    """Get an instance of the NVIDIA Control Panel interface."""
    if NVIDIA_CONTROL_PANEL_AVAILABLE:
        return NVIDIAControlPanel()
    else:
        raise ImportError("NVIDIA Control Panel integration not available")

# Export the main class
__all__ = ['NvidiaIntegration']
